...
# 			# 	st.pyplot()
		...
		if data is not None:
			...
			# st.checkbox("Select Target column")
			...
			# 	CV=model_selection.cross_val_score(model,x,y,cv=kfold,scoring='accuracy')
			# 	modelNames.append(name)
			# 	modelMeans.append(CV.mean())
			# 	modelSTD.append(CV.std())

			# 	AccuracyResults={"Model Name":name,"Model accuracy":CV.mean(),"Model STD":CV.std()}
			# 	OverallModel.append(AccuracyResults)

			# 	#plotting

			# if st.checkbox("Table of Metrics"):
			# 	st.dataframe(pd.DataFrame(zip(modelNames,modelMeans,modelSTD),columns=['Model Name','Model Accuracry','Model STD']))
